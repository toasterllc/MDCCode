...
static Args parseArgs(int argc, const char* argv[]) {
    ...
    } else if (args.cmd == lower(TestCmd)) {
...
static void Test() {
    ...
    const fs::path path = "/Volumes/Photon/a";
    const int fd = open(path.c_str(), O_RDWR);
    assert(fd >= 0);
    
    {
        int ir = fcntl(fd, F_NOCACHE, 1);
        assert(ir == 0);
    }
    
    {
        ...
        ir = pwrite(fd, &cmd, sizeof(cmd), 0);
        assert(ir == sizeof(cmd));
        
        ir = pread(fd, resp, sizeof(resp), 0);
        assert(ir == sizeof(resp));
    }
}
