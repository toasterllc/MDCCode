...
//
//StaticPrint(sizeof(std::chrono::seconds::rep));
...
int main() {
    ...
    using TocksFreq = std::ratio<32768, 64>;
    ...
    using Tocks = std::chrono::duration<intmax_t, TocksPeriod>;
    using Ticks = std::chrono::duration<intmax_t, TicksPeriod>;
    
    static constexpr Tocks TimerIntervalTocks(0x10000);
    static constexpr Ticks TimerIntervalTicks(std::chrono::duration_cast<Ticks>(TimerIntervalTocks));
    static_assert(Tocks(TimerIntervalTicks) == TimerIntervalTocks);
