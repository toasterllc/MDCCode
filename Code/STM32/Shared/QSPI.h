...
//    };
    ...
    static void Command(const QSPI_CommandTypeDef& cmd) {
        AssertArgNoLED(_Config);
        AssertArgNoLED(cmd.DataMode == QSPI_DATA_NONE);
        AssertArgNoLED(!cmd.NbData);
        ...
        if (cmd.DummyCycles) {
            ...
            if (_Config->mode == Mode::Single) {
                ...
                AssertArgNoLED(!(cmd.DummyCycles % 2));
    ...
    static void Read(const QSPI_CommandTypeDef& cmd, void* data) {
        ...
        if (_Config->align == Align::Word) {
            AssertArgNoLED(!((uintptr_t)data % sizeof(uint32_t)));
            AssertArgNoLED(!(len % sizeof(uint32_t)));
        }
        AssertLED(!_Busy);
