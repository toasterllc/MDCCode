...
//    }
    ...
    {
        _mspTest.config(GPIO_MODE_INPUT, GPIO_NOPULL, GPIO_SPEED_FREQ_LOW, 0);
...
//        }
    ...
    uint16_t i_WriteJmbIn16(uint16_t dataX)
    {
        uint16_t sJMBINCTL = INREQ;
        uint16_t sJMBIN0 = dataX;
        uint16_t Timeout = 0;
...
//        TCK_dir( 1 );
    ...
    uint16_t GetCoreID(void) {
        ...
        for (i = 0; i < MAX_ENTRY_TRY; i++)
        {
            ...
            ResetTAP();
            // shift out JTAG ID
            JtagId = IR_Shift(IR_CNTRL_SIG_CAPTURE);  
