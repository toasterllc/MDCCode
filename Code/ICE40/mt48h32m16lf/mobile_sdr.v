...
*  
*  Copyright © 2001-2008 Micron Technology, Inc. All rights reserved.
*
*
* Rev  Author          Date        Changes
* ---  --------------------------  ---------------------------------------
* 6.10 clk             02/26/2008  - Fix bank interrupt/ap bug                                     
* 6.00 clk             02/10/2008  - Add write burst mode support                                  
* 5.80 clk             12/10/2008  - Updated Timing parameters for Y36N                            
* 5.70 clk             09/29/2008  - Added data display to Debug output                            
* 5.60 clk             07/25/2008  - Fixed cmd execution problem with load mode cmds when DEBUG=0. Fixed concurrent bank issue.
* 5.50 clk             05/09/2008  - Removed CKE dependacy for initialization         
* 5.40 clk             05/27/2008  - fixed iterator problem with recursive task calls 
* 5.30 clk             04/24/2008  - Added Y35M speed grades, removed old speed grades. Moved 512Mb to Y37M, Y47M is new 512Mb
...
`ifdef MOBILE_SDR_INIT_VAL
    // DKECK: Initialize the entire memory to `MOBILE_SDR_INIT_VAL
    //   (Originally we added this to ensure that the DQM lines work as expected,
    //   because this module still prints debug read/write messages even when
    //   DQM should mask the read/writes. In the write case, these initialization
    //   values show us (via the debug messages) that when DQM is masking the
    //   write, this module just writes the value that's already stored in the
    //   memory, so DQM is working.)
    integer icounter;
    initial begin
        $display("[mobile_sdr] Initializing memory to 0x%x...", `MOBILE_SDR_INIT_VAL);
        for (icounter=0; icounter<(1<<full_mem_bits); icounter++) begin
            mem_array[icounter] = `MOBILE_SDR_INIT_VAL;
        end
        $display("[mobile_sdr] Finished initializing memory");
    end
`endif
...
//------------- Integer Declarations --------------
    ...
    integer                      warnings                                 ;
    integer                      errors                                   ;
    integer                      burst_length_wr                          ;
    integer                      burst_length_rd                          ;
...
//------------- Wire Declarations --------------
    ...
    wire                         addr_10                        ;
    wire      [ 5 : 0]           command                        ;
    reg [1:8*16] command_display;
    always @ (command)
    begin
        case (command)
            6'b100111 :     command_display = "NOP             ";
            6'b100011 :     command_display = "ACTIVATE        ";
            6'b100101 :     command_display = "READ            ";
            6'b110101 :     command_display = "READ_AP         ";
            6'b000101 :     command_display = "READ_SUSPEND    ";
            6'b010101 :     command_display = "READ_AP_SUSPEND ";
            6'b100100 :     command_display = "WRITE           ";
            6'b110100 :     command_display = "WRITE_AP        ";
            6'b000100 :     command_display = "WRITE_SUSPEND   ";
            6'b010100 :     command_display = "WRITE_AP_SUSPEND";
            6'b100110 :     command_display = "BURST_TERMINATE ";
            6'b001111 :     command_display = "POWER_DOWN_CI   ";
            6'b000111 :     command_display = "POWER_DOWN_NOP  ";
            6'b000110 :     command_display = "DEEP_POWER_DOWN ";
            6'b100010 :     command_display = "PRECHARGE       ";
            6'b110010 :     command_display = "PRECHARGE_ALL   ";
            6'b100001 :     command_display = "AUTO_REFRESH    ";
            6'b000001 :     command_display = "SELF_REFRESH    ";
            6'b100000 :     command_display = "LOAD_MODE       ";
            6'b011111 :     command_display = "CKE_DISABLE     ";
            default   :     command_display = "UNSUPPORTED!!!!!";
        endcase
    end
...
//---------------------- Command Selection ----------------------
    ...
    assign addr_10 = addr[10] & ((cke_q & ~cs_n &  ras_n & ~cas_n &  we_n & ~(burst_length_rd == `PAGE_SIZE) ) |  // Read w/ap
                                 (cke_q & ~cs_n &  ras_n & ~cas_n & ~we_n & ~(burst_length_wr == `PAGE_SIZE) ) |  // Write w/ap
...
//---------------------- Mode Register Selection ----------------------
    ...
    begin
        ...
        if (addr[2:0] == 3'b000) begin
            ...
            burst_length_rd = `PAGE_SIZE ;
            burst_length_wr = `PAGE_SIZE ;
        end else begin
            burst_length_rd = 0 ;
            burst_length_wr = 0 ;
        end
    ...
    task column_burst_order_rd;
        integer i ;
    begin
        burst_count = 0 ;
        for (i=0; i<burst_length_rd; i=i+1) begin
            if (burst_length_rd == `PAGE_SIZE) begin
    ...
    task column_burst_order_wr;
        integer i ;
    begin
        burst_count = 0 ;
        for (i=0; i<burst_length_wr; i=i+1) begin
            if (burst_length_wr == `PAGE_SIZE) begin
                if (burst_type == 1'b0) begin
                    // $display("HERRO: 0x%x", addr);
                    col_addr_burst_order[i] = addr[COL_BITS-1:0] + burst_count ;
                end else if (burst_type == 1'b1) begin
                    col_addr_burst_order[i] = {COL_BITS{1'bx}} ;
                end
            end else if (burst_length_wr == 1) begin
                if (burst_type == 1'b0) begin
                    col_addr_burst_order[i] = {addr[COL_BITS-1:3], addr[2:0]} ;
                end else if (burst_type == 1'b1) begin
                    col_addr_burst_order[i] = {addr[COL_BITS-1:3], addr[2:0]} ;
                end
            end else if (burst_length_wr == 2) begin
                if (burst_type == 1'b0) begin
                    col_addr_burst_order[i] = {addr[COL_BITS-1:3], addr[2:1], (burst_count[0] + addr[0])} ;
                end else if (burst_type == 1'b1) begin
                    col_addr_burst_order[i] = {addr[COL_BITS-1:3], addr[2:1], (burst_count[0] ^ addr[0])} ;
                end
            end else if (burst_length_wr == 4) begin
                if (burst_type == 1'b0) begin
                    col_addr_burst_order[i] = {addr[COL_BITS-1:3], addr[2], (burst_count[1:0] + addr[1:0])} ;
                end else if (burst_type == 1'b1) begin
                    col_addr_burst_order[i] = {addr[COL_BITS-1:3], addr[2], (burst_count[1:0] ^ addr[1:0])} ;
                end
            end else if (burst_length_wr == 8) begin
                if (burst_type == 1'b0) begin
                    col_addr_burst_order[i] = {addr[COL_BITS-1:3], (burst_count[2:0] + addr[2:0])} ;
                end else if (burst_type == 1'b1) begin
                    col_addr_burst_order[i] = {addr[COL_BITS-1:3], (burst_count[2:0] ^ addr[2:0])} ;
...
//---------------------- Memory Address Queue ----------------------
    ...
    begin
        ...
        if (burst_length_rd < `PAGE_SIZE) begin
            for (i=burst_length_rd; i< (burst_length_rd+cas_latency-1);i=i+1) begin
    ...
    begin
        ...
        if (burst_length_wr < `PAGE_SIZE) begin
            for (i=burst_length_wr; i<(burst_length_wr+cas_latency-1);i=i+1) begin
                bank_access_q[i] = 'bz ;
                row_access_q[i] = 'bz ;
                column_access_q[i] = 'bz ;
                column_access_valid_q[i] = 2'b00 ;
    ...
    wire[COL_BITS - 1 : 0] column_access_q0 = column_access_q[0];
    wire[COL_BITS - 1 : 0] col_addr_burst_order0 = col_addr_burst_order[0];
        ...
        for (i=0; i<`BANKS; i=i+1) begin
            if (ap_set[i]) begin
                if (tm_bank_write[i] > tm_bank_read[i]) begin
                    if ((ck_cntr_bank_write[i]         >= burst_length_wr) &
                        (($time - tm_bank_activate[i]) >= tRAS        )  ) begin
                        precharge_cmd_func(i, tWRa) ;
                    end
                end else begin
                    if ((ck_cntr_bank_read[i]          >= burst_length_rd) &
                        (($time - tm_bank_activate[i]) >= tRAS        )  ) begin
...
//            $display("DATA_WRITTEN to memory at time %t: %0x", $time, mdata);
            if (DEBUG == 1'b1) begin
                $sformat (msg, " CAS  : WRITE - Bank = 0x%h, Row = 0x%h, Col = 0x%h, Data = 0x%h at time %t",bank_access_q[0], row_access_q[0], column_access_q[0], mdata, $time); NOTE(msg);
            end
        ...
        if (column_access_valid_q[0] == 2'b10)  begin
            ...
            if (DEBUG == 1'b1) begin
                $sformat (msg, " CAS  : READ - Bank = 0x%h, Row = 0x%h, Col = 0x%h, Data = 0x%h at time %t",bank_access_q[0], row_access_q[0], column_access_q[0], Dq_out, $time); NOTE(msg);
            end
        end else begin
            Dq_out = 'bz ;
        end
...
//-------------------------------- Precharge Tasks -------------------------------
    ...
    begin
        if (delay > 0) begin
            ...
            if (interrupt_bank == bank) begin
                interrupt_bank             <= #delay 2'hX            ;
                interrupt_write_ap_n[bank] <= #delay 2'h2            ;
                interrupt_read_ap_n[bank]  <= #delay 2'h1            ;
            end
            ...
            if (interrupt_bank == bank) begin
                interrupt_bank             = 2'hX                    ;
                interrupt_write_ap_n[bank] = 2'h2                    ;
                interrupt_read_ap_n[bank]  = 2'h1                    ;
            end
            // Precharge interrupt a read command to the same bank
            if (bank_access_q[0] == bank) begin
                for (i=(cas_latency-1); i<((cas_latency-1)+burst_length_rd); i=i+1) begin
...
//-------------------------------- Write Tasks -------------------------------
    ...
    begin
        if (DEBUG == 1'b1) begin
            $sformat (msg, " WR   : WRITE - Bank = 0x%h, Row = 0x%h, Col = 0x%h", ba, activate_row[ba], addr); NOTE(msg);
        end
    end
    endtask
...
//-------------------------------- Read Tasks -------------------------------
    ...
    task read_cmd_func;
        integer i ;
    begin
        column_burst_order_rd          ;
        if (column_access_valid_q[0] == 2'b01) begin
            for (i=0; i<(burst_length_rd+cas_latency-1); i=i+1) begin
                column_access_valid_q[i] = 2'b00 ;
            end
        end
    ...
    begin
        if (DEBUG == 1'b1) begin
            $sformat (msg, " RD   : READ - Bank = 0x%h, Row = 0x%h, Col = 0x%h", ba, activate_row[ba], addr); NOTE(msg);
        end
    end
    endtask
...
//------------- System clock --------------
    ...
    always@(posedge Sys_clk) begin
        ...
        if (command_sequence_error == 0) begin
            ...
            if (command == ACTIVATE        ) begin activate_cmd_display            ; end
            if (command == READ            ) begin read_cmd_display                ; end
            if (command == READ_AP         ) begin read_ap_cmd_display             ; end
            if (command == READ_SUSPEND    ) begin clock_suspend_cmd_display       ; end
            if (command == READ_AP_SUSPEND ) begin clock_suspend_cmd_display       ; end
            if (command == WRITE           ) begin write_cmd_display               ; end
            if (command == WRITE_AP        ) begin write_ap_cmd_display            ; end
            if (command == WRITE_SUSPEND   ) begin clock_suspend_cmd_display       ; end
            if (command == WRITE_AP_SUSPEND) begin clock_suspend_cmd_display       ; end
            if (command == BURST_TERMINATE ) begin burst_term_cmd_display          ; end
            if (command == AUTO_REFRESH    ) begin auto_refresh_cmd_display        ; end
            if (command == PRECHARGE       ) begin precharge_cmd_display(ba)       ; end
            if (command == PRECHARGE_ALL   ) begin precharge_all_cmd_display       ; end
            if (command == LOAD_MODE       ) begin load_mode_cmd_display(ba, addr) ; end
            if ((~read_write_in_progress     ) &
                (command != READ_SUSPEND     ) &
                (command != READ_AP_SUSPEND  ) &
                (command != WRITE_SUSPEND    ) &
                (command != WRITE_AP_SUSPEND )  ) begin
                if (command == SELF_REFRESH   ) begin self_refresh_cmd_display    ; end
                if (command == POWER_DOWN_CI  ) begin power_down_cmd_display      ; end
                if (command == POWER_DOWN_NOP ) begin power_down_cmd_display      ; end
                if (command == DEEP_POWER_DOWN) begin deep_power_down_cmd_display ; end
            end else begin
                if (command == SELF_REFRESH   ) begin clock_suspend_cmd_display   ; end
                if (command == POWER_DOWN_CI  ) begin clock_suspend_cmd_display   ; end
                if (command == POWER_DOWN_NOP ) begin clock_suspend_cmd_display   ; end
                if (command == DEEP_POWER_DOWN) begin clock_suspend_cmd_display   ; end
            end
        end
        command_sequence_error = 0 ;
