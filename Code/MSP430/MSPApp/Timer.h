...
public:
    ...
    // Returns whether we actually waited for the full duration (true), or if we were interrupted (false)
    static bool Wait() {
//        Assert(_State.request.reset);
        ...
        // If we weren't given a time, wait indefinitely to be interrupted
        if (!_State.request.time) {
            // Wait until we get a request with a valid time
            T_Scheduler::Wait([] { return _State.request.reset; });
            return false;
        }
        ...
        if (deltaTicks) {
            ...
            if (_State.rtc.waiting) {
                ...
                if (_State.request.reset) return false;
                // Update deltaTicks
