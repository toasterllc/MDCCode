...
inline void _FileTimestampSet(const std::filesystem::path& filePath, Time::Instant timestamp) {
...
inline void ExportJPEGPNG(Toastbox::Renderer& renderer, const ImageRecord& rec, const Image& image,
    ...
    assert(&fmt==&Formats::JPEG || &fmt==&Formats::PNG);
...
inline void Export(Toastbox::Renderer& renderer, const ImageRecord& rec, const Image& image,
    ...
    if (&fmt==&Formats::JPEG || &fmt==&Formats::PNG) {
        ExportJPEGPNG(renderer, rec, image, fmt, filePath);
    
    } else if (&fmt == &Formats::DNG) {
        ExportDNG(rec, image, filePath);
